resource "ibm_is_instance" "consul_instance" {
  ...
  user_data = templatefile("${path.module}/installer.sh", { consul_version = var.consul_version, acl_token = var.acl_token, encrypt_key = var.encrypt_key, vpc_name = var.vpc_name, domain = var.domain, zone = data.ibm_is_zones.regional_zones.zones[0], region = var.region })
}
